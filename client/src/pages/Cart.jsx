    ...
    <section className="page-shell container cart-page">
      ...
      <div className="cart-layout">
        <div className="cart-items">
          {cart.map((item) => <article className="cart-item" key={item.id}><img src={item.image} alt={item.name}/><div className="cart-item-copy"><span>{item.category}</span><Link to={`/products/${item.id}`}>{item.name}</Link><strong>{formatNaira(item.price)}</strong></div><div className="quantity"><button onClick={() => decreaseQuantity(item.id)}><Minus/></button><b>{item.quantity}</b><button onClick={() => increaseQuantity(item.id)}><Plus/></button></div><strong className="line-total">{formatNaira(item.price * item.quantity)}</strong><button className="remove-btn" onClick={() => removeFromCart(item.id)} aria-label={`Remove ${item.name}`}><Trash2/></button></article>)}
        </div>
