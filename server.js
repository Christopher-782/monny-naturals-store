...
const HAS_ADMIN_CONFIG = Boolean(process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD && (process.env.JWT_SECRET || process.env.ADMIN_TOKEN_SECRET));
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || (IS_VERCEL ? '' : 'admin@monnynaturals.local');
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || (IS_VERCEL ? '' : 'MonnyAdmin123!');
const TOKEN_SECRET = process.env.JWT_SECRET || process.env.ADMIN_TOKEN_SECRET || (IS_VERCEL ? '' : 'change-this-admin-token-secret-before-production');
...
async function writeContent(nextContent) {
  if (!blobConfigured()) {
    if (IS_VERCEL) {
      throw new Error('Vercel Blob is not configured. Create a public Blob store in this Vercel project first.');
    }
  ...
  try {
    ...
    console.warn('Blob history cleanup skipped:', cleanupError?.message || cleanupError);
  }
}
...
function requireAdmin(req, res, next) {
  ...
  try {
    ...
    res.status(401).json({ error: 'Admin session has expired. Please log in again.' });
  }
}
...
function validateAndNormalizeContent(input) {
  ...
  if (!Array.isArray(input.products) || !Array.isArray(input.categories) || !Array.isArray(input.heroSlides)) {
    throw new Error('Products, categories and hero slides are required.');
  }
  if (!input.heroSlides.length) throw new Error('Keep at least one hero slide.');

  const categoryNames = input.categories.map((c) => String(c?.name || '').trim()).filter(Boolean);
  if (categoryNames.length !== input.categories.length) throw new Error('Every category needs a name.');
  if (new Set(categoryNames.map((n) => n.toLowerCase())).size !== categoryNames.length) throw new Error('Category names must be unique.');
  ...
  const products = input.products.map((product) => {
    ...
    if (!categoryNames.includes(category)) throw new Error(`Product “${name}” uses an unknown category.`);
    return {
      ...product,
      name,
      category,
  ...
  const store = {
    ...(input.store || {}),
    brandName: String(input.store?.brandName || 'Monny Naturals').trim() || 'Monny Naturals',
    tagline: String(input.store?.tagline || ''),
    whatsappNumber: String(input.store?.whatsappNumber || '').replace(/[^0-9+]/g, ''),
  ...
  homepage.heroRotationSeconds = normalizeNumber(homepage.heroRotationSeconds, 2, 2, 12);
  ...
  return { ...input, products, categories, heroSlides, store, homepage, testimonials, benefits, catalogSections, theme };
}
...
app.post('/api/admin/login', (req, res) => {
  if (IS_VERCEL && !HAS_ADMIN_CONFIG) {
    return res.status(503).json({ error: 'Admin credentials are not configured in Vercel Environment Variables.' });
  }
  const { email, password } = req.body || {};
  if (!safeEqualText(email, ADMIN_EMAIL) || !safeEqualText(password, ADMIN_PASSWORD)) {
    return res.status(401).json({ error: 'Invalid email or password.' });
  }
  const token = signToken({ email: ADMIN_EMAIL, role: 'admin', exp: Date.now() + TOKEN_LIFETIME_MS });
...
app.put('/api/admin/content', requireAdmin, async (req, res) => {
  try {
    ...
    const isValidation = /required|needs|unique|unknown|invalid|keep at least/i.test(message);
...
app.post('/api/admin/upload', requireAdmin, async (req, res) => {
  try {
    ...
    if (typeof data !== 'string' || !data) return res.status(400).json({ error: 'Choose an image to upload.' });

    const base64 = data.includes(',') ? data.slice(data.indexOf(',') + 1) : data;
    const buffer = Buffer.from(base64, 'base64');
    if (!buffer.length) return res.status(400).json({ error: 'The uploaded image is empty or invalid.' });
    ...
    if (buffer.length > 2600 * 1024) return res.status(400).json({ error: 'Image is still too large after optimisation. Choose a smaller image.' });

    const cleanBase = path.basename(String(name || 'image'), path.extname(String(name || 'image')))
    ...
    if (blobConfigured()) {
      ...
      return res.json({ ok: true, url: blob.url, filename: blob.pathname, storage: 'vercel-blob' });
    }

    if (IS_VERCEL) {
      return res.status(503).json({ error: 'Vercel Blob is not configured. Create a public Blob store in this Vercel project first.' });
    }

    fs.mkdirSync(UPLOAD_DIR, { recursive: true });
    fs.writeFileSync(path.join(UPLOAD_DIR, filename), buffer);
    res.json({ ok: true, url: `/uploads/${filename}`, filename, storage: 'local-files' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: error?.message || 'Could not upload image.' });
  }
});

app.use((error, _req, res, next) => {
  if (error && error.type === 'entity.too.large') {
    return res.status(413).json({ error: 'Upload request is too large. Try a smaller image.' });
  }
  next(error);
});

const serveProductionClient = process.env.NODE_ENV === 'production' || process.argv.includes('--production') || IS_VERCEL;
...
app.listen(PORT, () => {
  ...
  if (!process.env.ADMIN_EMAIL || !process.env.ADMIN_PASSWORD || !(process.env.JWT_SECRET || process.env.ADMIN_TOKEN_SECRET)) {
    console.warn('Using development admin defaults. Configure production environment variables before deploying.');
  }
  if (IS_VERCEL && !blobConfigured()) {
    console.warn('Vercel Blob is not connected. The storefront can read bundled content, but CMS saves/uploads are disabled.');
  }
});
